...
\hlkwd{barplot}\hlstd{(hellingerDist,} \hlkwc{names.arg} \hlstd{=} \hlkwd{colnames}\hlstd{(ANES),}
        \hlkwc{main} \hlstd{=} \hlstr{"Difference Between Conservatives and Liberals"}\hlstd{,}
...
Plotting the poster mean of membership in the conservative bloc below, we can see a fair amount of intra-individual mixing. We can see that 139 out of the 279 individuals have estimated memberships of at least 40\% in both the conservative and liberal blocs.
...
\hlcom{# plot posterior 80% CI}
\hlstd{ci_90} \hlkwb{=} \hlkwd{qbeta}\hlstd{(}\hlnum{.9}\hlstd{, out.permute}\hlopt{$}\hlstd{phi[index,}\hlnum{1}\hlstd{],} \hlkwd{rowSums}\hlstd{(out.permute}\hlopt{$}\hlstd{phi[index,}\hlkwd{c}\hlstd{(}\hlnum{2}\hlopt{:}\hlnum{3}\hlstd{)]))}
\hlstd{ci_10} \hlkwb{=} \hlkwd{qbeta}\hlstd{(}\hlnum{.1}\hlstd{, out.permute}\hlopt{$}\hlstd{phi[index,}\hlnum{1}\hlstd{],} \hlkwd{rowSums}\hlstd{(out.permute}\hlopt{$}\hlstd{phi[index,}\hlkwd{c}\hlstd{(}\hlnum{2}\hlopt{:}\hlnum{3}\hlstd{)]))}
...
\hlkwd{legend}\hlstd{(}\hlstr{"bottomright"}\hlstd{,} \hlkwc{legend} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"posterior mean"}\hlstd{,} \hlstr{"80% CI"}\hlstd{),} \hlkwc{pch} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{19}\hlstd{,} \hlnum{NA}\hlstd{),}
