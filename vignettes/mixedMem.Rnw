...
# Number of variables
J <- 19 
# we only have one repeated measure for each of the variables
Rj <- rep(1, J)
...
Notice that the lower bound (our objective function) is \Sexpr{round(computeELBO(initial),2)} at the initialized points and is \Sexpr{round(computeELBO(out),2)} at convergence. On my laptop (quad-core 2.4 GHZ with 12 GB of RAM), fitting the entire model took only \Sexpr{time[3]} seconds, significantly faster than a full MCMC analysis.  
...
Loss = \sum_i^{Total} \sum_j^J \frac{\hat \alpha_k}{\hat \alpha_0} \sum_v^{V_j}(\hat \theta_{j,k,v} - \theta_{j,k,v})^2
\end{equation}
...
Interpretation of the fitted model is highly context specific, but we show a few visualizations and tools which may be of general interest below.
\subsubsection{Visual Representation of $\hat \theta$}
We can now plot our estimated $\hat \theta$ values and compare them to the values found in the original analysis. In the plots shown below, each row of plots represents an individual question, the columns represent each of the identified sub-populations and the plots show the sub-populations's response probability for each variable. The black bars indicate the quantities estimated using \texttt{mixedMem} and the green dots indicate the quantites reported by \cite[Gross and Manrique-Vallier, CH6]{Airoldi2014Handbook}. Note that Gross and Manrique-Vallier do not report values for group 3 for reasons which will become more clear when discussed below. Recall that 0 indicates ``agree" and 2 indicates ``disagree".

<<thetaPlot, fig.height = 7, fig.width = 7, fig.align = 'center', fig.cap = "Variational Estimates shown in black, GMV estimates shown in green">>=
...
We can see that for sub-population 2, the statements most likely to elicit agreement are statements \texttt{EQ7} (``One of the big problems in this country is that we don't give everyone an equal chance
"), \texttt{IND6} (``Even if people try hard, they often cannot reach their goals
...
We also observe, that sub-population 3 has a much higher propensity to respond ``can't decide" then sub-populations 1 or 2. The 5 individuals with particularly large membership in group 3 are responded ``can't decide" 42 times. This is particularly salient since there are only 99 ``can't decide" responses in the entire sample. Thus, sub-population 3 could be identified as the undecided bloc. 
...
barplot(hellingerDist, names.arg = colnames(ANES),
...
For the groups memberships for individuals, we use the posterior mean, $\frac{\hat \phi_i}{\sum_k \hat \phi_{i,k}}$ as point estimates for $\lambda_i$. In general, we see a high level of intra-individual mixing.

<<echo = FALSE>>=
estimatedLambda = out.permute$phi/rowSums(out.permute$phi)
@

Plotting the poster mean of membership in the conservative bloc below, we can see a fair amount of intra-individual mixing. We can see that \Sexpr{sum(estimatedLambda[,1]>.4 & estimatedLambda[,2] >.4)} out of the \Sexpr{Total} individuals have estimated memberships of at least 40\% in both the conservative and liberal blocs.
 This is not particularly suprising since we observed relatively large values of  $\hat \alpha_1$ and $\hat \alpha_2$.
...
We can also plot the ``empirical CDF" of estimated membership in conservative bloc. Since the marginal distribution of a Dirichlet distribution is a beta, we can also plot the 80\% credible interval for the posterior membership in the conservative bloc. We observe that there is still a relatively large amount of uncertainty in the membership of each individual.


<<fig.width = 5, fig.align = 'center', fig.height = 5>>=
...
# plot posterior 80% CI
ci_90 = qbeta(.9, out.permute$phi[index,1], rowSums(out.permute$phi[index,c(2:3)]))
ci_10 = qbeta(.1, out.permute$phi[index,1], rowSums(out.permute$phi[index,c(2:3)]))
lines(ci_90, lty = 2, col = "red")
lines(ci_10, lty = 2, col = "red")
legend("bottomright", legend = c("posterior mean", "80% CI"), pch = c(19, NA),
